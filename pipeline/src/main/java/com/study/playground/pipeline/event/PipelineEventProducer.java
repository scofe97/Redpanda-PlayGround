 ...
 *
 * PipelineEngine이 Job 실행 상태 변경·실행 완료 시점에 이 클래스를 호출한다.
...
public class PipelineEventProducer {
    ...
    private static final String JOB_EXECUTION_CHANGED_EVENT_TYPE = "PIPELINE_STEP_CHANGED";
    ...
    /**
     * 파이프라인 Job 실행의 상태 변경 이벤트를 발행한다.
     *
     * Job 실행 로그를 함께 포함시키는 이유는 클라이언트가 별도 조회 없이
     * SSE 이벤트 하나로 현재 상태와 로그를 모두 렌더링할 수 있도록 하기 위해서다.
     */
    public void publishJobExecutionChanged(
        ...
        var event = PipelineStepChangedEvent.newBuilder()
                ...
                .setStepName(jobExecution.getJobName())
                .setStepType(jobExecution.getJobType().name())
        ...
        eventPublisher.publish(
                ...
                , JOB_EXECUTION_CHANGED_EVENT_TYPE
