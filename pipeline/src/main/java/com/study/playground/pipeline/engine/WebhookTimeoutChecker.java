 ...
 * 설정된 시간 이상 대기 중인 Job을 타임아웃 처리한다.
 *
...
public class WebhookTimeoutChecker {
    ...
    public void checkTimeouts() {
        int timeoutMinutes = props.webhookTimeoutMinutes();
        var timedOutJobs = jobExecutionMapper.findWaitingWebhookOlderThan(timeoutMinutes);
        ...
        for (var je : timedOutJobs) {
            ...
            String errorMsg = String.format("Webhook timeout: no callback received within %d minutes", timeoutMinutes);
            ...
            if (execution != null) {
                ...
                TraceContextUtil.executeWithRestoredTrace(
                        ...
                        , Map.of("pipeline.execution.id", execution.getId().toString()
                                , "timeout.minutes", String.valueOf(timeoutMinutes))
                        , () -> {
