...
### 1-2. Spring Boot 로그 수집

Spring Boot는 Docker 컨테이너가 아닌 호스트에서 실행되므로 Alloy가 Docker 소켓으로 로그를 수집하지 못한다. GCP 환경에서는 Loki4j Logback Appender로 Loki에 직접 전송한다([02-setup-and-usage.md](./02-setup-and-usage.md) 섹션 7 참조). 로컬 환경에서는 터미널 또는 IntelliJ 콘솔에서 확인한다. 트레이스와 메트릭은 OTLP/Prometheus로 정상 수집된다.
...
```bash
# 로컬
cd infra/docker/local && docker compose -f docker-compose.monitoring.yml restart tempo
docker ps --filter name=playground-tempo   # Status: healthy 확인

# GCP (Server 3)
gcloud compute ssh dev-server-3 --zone=asia-northeast3-a \
  --command="cd ~/deploy/server-3 && docker compose restart tempo"
```
...
grep -A2 "docker.sock" infra/docker/local/docker-compose.monitoring.yml
```
...
```bash
# 각 서버 컨테이너 상태
for s in dev-server dev-server-2 dev-server-3; do
  echo "=== $s ===" && gcloud compute ssh $s --zone=asia-northeast3-a \
    --command="docker ps --format 'table {{.Names}}\t{{.Status}}'"
done

# Server 3 모니터링 상태
gcloud compute ssh dev-server-3 --zone=asia-northeast3-a \
  --command="docker stats --no-stream && docker exec playground-tempo du -sh /var/tempo/"
```
...
```bash
# Prometheus TSDB 크기
# 로컬: localhost:29090, GCP: 34.22.78.240:9090
curl -s http://localhost:29090/api/v1/status/tsdb | jq '.data.headStats'

# Tempo/Loki 디스크 (GCP Server 3)
gcloud compute ssh dev-server-3 --zone=asia-northeast3-a --command="\
