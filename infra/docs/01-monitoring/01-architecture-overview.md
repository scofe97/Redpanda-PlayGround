...
GCP에서는 **Alloy sidecar + central 패턴**을 사용한다. 로컬 개발자 맥에서도 `infra/docker/local/`의 Alloy가 실행되어 Spring Boot OTLP 트레이스를 수신한 뒤 Server 3 central로 릴레이한다. 각 GCP 서버에는 Alloy sidecar가 Docker 소켓으로 로컬 로그를 수집하고, Server 3의 central Alloy가 모든 OTLP 트레이스를 수신하여 노이즈 필터링을 한 곳에서 처리한다. 필터 규칙 변경 시 Server 3 Alloy만 재시작하면 된다.
...
로컬 Docker Compose로 모든 서비스를 단일 호스트에서 실행한다. compose 파일은 `infra/docker/local/`에 위치한다.
...
| 항목 | 로컬 (`infra/docker/local/`) | GCP (`infra/docker/deploy/server-*`) |
|------|----------------------|-------------------------------|
...
로컬 compose 파일은 `infra/docker/local/`에, 공통 설정(monitoring, connect 등)은 `infra/docker/shared/`에 위치한다. GCP deploy 디렉토리(`infra/docker/deploy/server-*`)는 scp로 서버에 복사하는 self-contained 구조이므로 각 서버 디렉토리에 필요한 설정이 모두 포함되어 있다.
