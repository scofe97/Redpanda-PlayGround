...
각 Job은 `post.success`/`post.failure` 블록에서 curl로 `playground-connect:4195/jenkins-webhook/webhook/jenkins`에 결과를 콜백한다. 페이로드에는 `executionId`, `stepOrder`, `result`, `buildNumber`, `jobName`, `duration`, `url`이 포함된다. 이것이 Break-and-Resume 패턴의 핵심이다.
...
## 2. Webhook 전역 리스너 — 현재 구현

Part 2에서 검토한 방법 중 **RunListener (전역)**를 채택하여 K8s Helm values.yaml의 `initScripts.webhook-listener`로 구현했다. Docker 환경에서는 `webhook-listener.groovy`가 `init.groovy.d/`에 마운트된다.
...
run.parent.fullName으로 jobName 추출
  ...
  ↓
HTTP POST → CONNECT_WEBHOOK_URL
  ↓
Kafka (playground.webhook.inbound)
  ↓
Spring WebhookEventConsumer → PipelineEngine.resumeAfterWebhook()
```
...
**재시도 없음 (fire-and-forget)**: HTTP POST가 실패하면 로그만 남기고 끝난다. 개선 옵션 3가지가 있다.

| 옵션 | 방식 | 장점 | 단점 |
|------|------|------|------|
| A | Groovy 단 재시도 (3회, 지수 백오프) | 구현 간단 | 빌드 스레드 블로킹 |
| B | WebhookTimeoutChecker 폴링 보완 | 이미 존재하는 안전망 활용 | 타임아웃까지 지연 |
| C | Jenkins → Redpanda 직접 발행 (rpk) | at-least-once 보장 | 복잡도 증가 |

현재는 WebhookTimeoutChecker(옵션 B)가 최종 안전망 역할을 한다. 일정 시간 webhook이 안 오면 Jenkins API로 빌드 상태를 직접 조회한다. A + B 조합이 현 단계에서 적절하다.
...
`jobName`은 `run.parent.fullName`에서 나오므로 폴더 내 Job은 `folder/jobname` 형태가 된다. webhook 수신 쪽(`JenkinsWebhookHandler`)은 `EXECUTION_ID` + `STEP_ORDER`로 매칭하므로 jobName 형태가 바뀌어도 영향이 없다.
