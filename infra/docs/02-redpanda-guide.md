...
**주의**: Streams 모드에서 `http_server` input의 `address` 필드는 생략하고 공유 포트(4195)에 등록된다. 모든 webhook 경로(/webhook/jenkins, /webhook/gitlab)가 동일한 포트 4195를 공유하며, path 기반 라우팅으로 구분된다.
...
Connect 파이프라인의 CQRS 매핑(Command/Query/Event 분리)은 [../backend/04-patterns.md](../backend/04-patterns.md)에서 상세히 다룬다.

웹 UI를 통한 커넥터 동적 등록/관리 시나리오는 [../backend/04-patterns.md](../backend/04-patterns.md)를 참조한다.
...
- 프로젝트 내 관련 문서:
  - [../backend/04-patterns.md](../backend/04-patterns.md) — Connect 브릿지 패턴, 토픽/스키마 설계
