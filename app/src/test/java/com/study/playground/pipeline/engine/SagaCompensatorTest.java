...
class SagaCompensatorTest {
    ...
    void 스텝실패시_역순보상() throws Exception {
        ...
        inOrder.verify(stepMapper).updateStatus(eq(2L), eq(StepStatus.COMPENSATED.name()), anyString(), any());
        inOrder.verify(eventProducer).publishStepChanged(execution, step2, StepStatus.COMPENSATED);
        inOrder.verify(mockExecutor).compensate(execution, step1);
        inOrder.verify(stepMapper).updateStatus(eq(1L), eq(StepStatus.COMPENSATED.name()), anyString(), any());
        inOrder.verify(eventProducer).publishStepChanged(execution, step1, StepStatus.COMPENSATED);
    }
    ...
    void 보상실패시_계속진행() throws Exception {
        ...
        verify(stepMapper).updateStatus(eq(1L), eq(StepStatus.COMPENSATED.name()), anyString(), any());
        verify(eventProducer).publishStepChanged(execution, step1, StepStatus.COMPENSATED);
    }
