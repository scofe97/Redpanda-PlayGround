...
public record PipelineExecutionResponse(
     ...
     *
     * <p>{@code steps}가 null인 경우 빈 리스트로 처리하는 이유:
     * Mapper가 스텝이 없는 실행을 조회할 때 null을 반환할 수 있어, 클라이언트 NPE를 방지하기 위함이다.</p>
     *
     * @param execution 조회된 파이프라인 실행 도메인 객체
     * @param steps     실행에 속한 스텝 목록 (null 허용)
     */
     ...
     *
     * <p>파이프라인 시작 직후에는 스텝 정보가 아직 의미 없으므로 포함하지 않는다.
