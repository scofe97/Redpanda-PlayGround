...
기존 `01-architecture.md`나 `04-patterns.md`가 아키텍처와 패턴 중심이라면, 이 문서는 **"이 도메인이 뭘 하는 건지"**를 빠르게 파악하는 데 초점을 맞춘다.
...
| 주제 | 문서 |
|------|------|
| 전체 아키텍처 | [01-architecture.md](01-architecture.md) |
| 202 Accepted 패턴 | [04-patterns.md](04-patterns.md#async-accepted) |
...
핵심은 2단계에서 비즈니스 데이터(ticket, ticket_source)와 이벤트(outbox_event)가 **같은 트랜잭션**에 저장된다는 점이다. 이렇게 하면 서비스가 INSERT 직후 크래시하더라도 이벤트가 유실되지 않는다. Transactional Outbox 패턴에 대한 자세한 설명은 [01-architecture.md](01-architecture.md)를 참조한다.
...
- [01-architecture.md](01-architecture.md) — Transactional Outbox, 이벤트 직렬화 상세
- [04-patterns.md](04-patterns.md#async-accepted) — 202 Accepted 응답 패턴
...
WAITING_WEBHOOK 상태는 Break-and-Resume 패턴의 핵심이다. 이 상태에 진입하면 엔진은 스레드를 반납하고, 외부 콜백이 도착할 때까지 파이프라인 실행이 중단된다. 자세한 내용은 [04-patterns.md](04-patterns.md#break-and-resume)를 참조한다.
...
보상은 best-effort 방식이다. 개별 보상이 실패해도 나머지 보상은 계속 진행된다. 보상 실패한 스텝은 FAILED 상태로 남아 수동 개입이 필요함을 표시한다. SAGA 패턴의 설계 결정에 대한 자세한 내용은 [04-patterns.md](04-patterns.md#saga-orchestrator)를 참조한다.
...
- [04-patterns.md](04-patterns.md#saga-orchestrator) — SAGA 패턴 설계 결정과 트레이드오프
- [04-patterns.md](04-patterns.md#break-and-resume) — Break-and-Resume 패턴 상세
- [04-patterns.md](04-patterns.md#async-accepted) — 202 Accepted 응답 패턴
...
- [Ticket 도메인](#ticket-도메인-리뷰) — 파이프라인의 입력이 되는 Ticket 도메인
- [04-patterns.md](04-patterns.md#redpanda-connect) — Connect 브릿지 설정과 Bloblang 상세
- [04-patterns.md](04-patterns.md#break-and-resume) — Break-and-Resume 패턴과 CAS 동기화
