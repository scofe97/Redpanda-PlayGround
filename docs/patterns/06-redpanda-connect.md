...
Redpanda Connect가 `:4195` 포트에서 HTTP 요청을 받아 `ci-events` 토픽으로 전달한다. Spring 앱은 별도 webhook 엔드포인트를 구현할 필요 없이 기존 Kafka 컨슈머로 이벤트를 수신한다.
...
post {
    always {
        script {
            ...
            httpRequest(
                url: 'http://redpanda-connect:4195/events',
                httpMode: 'POST',
