...
Webhook 도메인은 자체 REST API를 노출하지 않는다. 외부 시스템의 HTTP 콜백은 Redpanda Connect가 수신하며(`http://localhost:4195/jenkins-webhook/webhook/jenkins`), 스프링 애플리케이션은 Kafka 컨슈머로만 웹훅을 처리한다.
