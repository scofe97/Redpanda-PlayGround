...
flowchart LR
    ...
    E -- "consume" --> G[JenkinsCommandConsumer]
    G -- "REST API" --> F[Jenkins]
    F -- "rpk produce" --> H[webhook.inbound]
    H -- "consume" --> D
...
| 3 | [Webhook 도메인](#webhook-도메인-리뷰) | 외부 시스템의 완료 콜백을 수신해서 Pipeline에 전달하는 브릿지. Jenkins가 rpk CLI로 Kafka에 직접 produce하고, WebhookEventConsumer가 소비하는 구조를 다룬다. |
...
| `PIPELINE_CMD_JENKINS` | `playground.pipeline.commands.jenkins` | → | PipelineCommandProducer → JenkinsCommandConsumer | Jenkins 빌드/배포 커맨드 |
| `WEBHOOK_INBOUND` | `playground.webhook.inbound` | ← | Jenkins(rpk) → WebhookEventConsumer | Jenkins 콜백 결과 수신 |
...
graph LR
    ...
    subgraph CMD["커맨드 발행"]
        ...
        T2 --> JCC[JenkinsCommandConsumer]
        JCC -->|REST API| JK[Jenkins]
    end

    subgraph CALLBACK["콜백 복귀"]
        JK -.->|rpk produce| T3["webhook.inbound"]
        T3 --> ENG
    end
    ...
    style JCC fill:#E8F5E9,stroke:#2E7D32,color:#333
...
graph TD
    subgraph CALLBACK["4. 외부 콜백 수신"]
        A["Jenkins<br>빌드 완료"] -->|rpk topic produce| C[Kafka<br>playground.webhook.inbound]
...
문제는 Jenkins가 직접 스프링 애플리케이션에 HTTP를 보내지 않는다는 점이다. Jenkins의 콜백 URL은 고정이고, 내부 서비스 주소는 변할 수 있으며, Jenkins가 Kafka 클라이언트를 직접 내장하기 어렵다. Webhook 도메인은 이 간극을 **rpk CLI**로 메운다. Jenkins 파이프라인이 완료되면 rpk를 실행하여 결과를 Kafka에 직접 produce하고, WebhookEventConsumer가 소비하여 PipelineEngine에 전달한다. Jenkins → rpk → Kafka(`webhook.inbound`) → WebhookEventConsumer → PipelineEngine 순서로 콜백이 전달된다.

---

### 핵심 개념

#### 현재 아키텍처: JenkinsCommandConsumer + rpk

현재는 Redpanda Connect 없이 앱과 Jenkins가 Kafka를 통해 직접 통신한다. 빌드 커맨드 방향은 앱이 직접 소비하고, 웹훅 결과 방향은 Jenkins가 rpk CLI로 직접 produce한다.

```mermaid
flowchart LR
    subgraph "Kafka → Jenkins (커맨드 전달)"
        K2[commands.jenkins] -->|consume| JCC[JenkinsCommandConsumer]
        JCC -->|POST /job/.../buildWithParameters| J2[Jenkins]
    end
    subgraph "Jenkins → Kafka (결과 전달)"
        J1[Jenkins] -->|rpk produce| K1[webhook.inbound]
        K1 -->|consume| WEC[WebhookEventConsumer]
    end

    style J1 fill:#fce4ec,stroke:#c62828,color:#333
    style J2 fill:#fce4ec,stroke:#c62828,color:#333
    style JCC fill:#e8f5e9,stroke:#2e7d32,color:#333
    style WEC fill:#e3f2fd,stroke:#1565c0,color:#333
    style K1 fill:#f3e5f5,stroke:#6a1b9a,color:#333
    style K2 fill:#f3e5f5,stroke:#6a1b9a,color:#333
```

**Kafka → Jenkins** (`JenkinsCommandConsumer`): `commands.jenkins` 토픽에서 `JENKINS_BUILD_COMMAND` 이벤트를 소비하여 Jenkins REST API(`POST /job/{jobName}/buildWithParameters`)를 호출한다. Basic Auth로 Jenkins에 인증하며, 이벤트 타입 필터링은 컨슈머 코드에서 처리한다.

**Jenkins → Kafka** (rpk CLI): Jenkins 파이프라인이 빌드/배포를 완료하면 Jenkinsfile의 `post` 블록에서 `rpk topic produce` 명령으로 결과를 `webhook.inbound` 토픽에 직접 produce한다. 페이로드는 `{ webhookSource: "JENKINS", executionId, stepOrder, result, ... }` 형식이다.
...
sequenceDiagram
    ...
    Note over Jenkins,Kafka: 1. Jenkins 빌드 완료 → rpk produce
    Jenkins->>Kafka: rpk topic produce webhook.inbound<br/>{webhookSource: "JENKINS", executionId, stepOrder, result, ...}
...
| Consumer | `JenkinsCommandConsumer` | `commands.jenkins` 소비, Jenkins REST API 호출 (빌드/배포 트리거) |
...
Webhook 도메인은 자체 REST API를 노출하지 않는다. Jenkins는 빌드/배포 완료 시 `rpk topic produce`로 직접 `webhook.inbound` 토픽에 결과를 발행하며, 스프링 애플리케이션은 Kafka 컨슈머(`WebhookEventConsumer`)로만 웹훅을 처리한다.

이 구조의 장점은 Jenkins 파이프라인이 Kafka에 직접 접근하므로 중간 브릿지 없이 레이턴시가 낮고, 별도 인프라 컴포넌트를 운영할 필요가 없다는 것이다.
