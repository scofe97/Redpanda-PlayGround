...
[Jenkins]
  빌드 완료 → POST http://playground-connect:4195/jenkins-webhook/webhook/jenkins
...
Jenkins Job 완료 시 webhook 콜백이 Redpanda Connect(포트 4195, 경로 /webhook/jenkins)를 거쳐 Kafka로 전달되고, Consumer가 파이프라인을 재개한다. Spring 애플리케이션에는 HTTP webhook 엔드포인트가 없으며, Redpanda Connect(`infra/docker/shared/connect/jenkins-webhook.yaml`)가 HTTP→Kafka 브릿지 역할을 한다. 커맨드 방향(App→Jenkins)도 마찬가지로 Connect(`infra/docker/shared/connect/jenkins-command.yaml`)가 Kafka→HTTP 브릿지 역할을 한다.
...
flowchart LR
    ...
    subgraph Connect["Redpanda Connect (:4195)"]
...
sequenceDiagram
    ...
    JK->>RC: curl POST :4195/jenkins-webhook/webhook/jenkins
