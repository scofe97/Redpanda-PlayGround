...
```java
// KafkaTopicConfig.java에서 명시적 정의
@Bean public NewTopic ticketTopic()    { return topic("playground.ticket", 3); }
@Bean public NewTopic pipelineTopic()  { return topic("playground.pipeline", 3); }
@Bean public NewTopic webhookTopic()   { return topic("playground.webhook.inbound", 3); }
@Bean public NewTopic auditTopic()     { return topic("playground.audit", 1); }
@Bean public NewTopic dlqTopic()       { return topic("playground.dlq", 1); }
```
...
graph TB
    ...
    subgraph Redpanda["Redpanda — :29092"]
        direction TB
        T_TICKET([playground.ticket])
        T_PIPELINE([playground.pipeline])
        T_WEBHOOK([playground.webhook.inbound])
        T_AUDIT([playground.audit])
    ...
    OUTBOX -- "produce" --> T_PIPELINE
    OUTBOX -- "produce" --> T_AUDIT

    %% 토픽 → Spring Consumer
    T_TICKET -- "consume" --> PIPELINE
    T_PIPELINE -- "consume" --> SSE
...
| **Command (앱→외부)** | PipelineEngine → Outbox → `playground.pipeline` → Connect → Jenkins REST | Connect가 Kafka→HTTP 변환 |
| **Query (앱→외부)** | ToolRegistry → Jenkins/GitLab/Nexus REST API 직접 호출 | 미들웨어 없음 (동기) |
| **Domain Event** | TicketService → Outbox → `playground.ticket` → PipelineEngine | Outbox 폴러가 DB→Kafka 변환 |
| **Realtime** | `playground.pipeline` → PipelineSseConsumer → SseEmitterRegistry → 브라우저 | SSE 단방향 스트리밍 |
