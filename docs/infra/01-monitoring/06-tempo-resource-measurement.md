...
Grafana 공식 문서는 Tempo에 대한 구체적인 CPU/메모리 최소 스펙을 명시하지 않는다. 대신 배포 모드별 특성과 커뮤니티 경험치를 기반으로 정리한다.

### 배포 모드 비교

| 항목 | Monolithic (이 프로젝트) | Microservices |
|------|------------------------|---------------|
| 구조 | 단일 바이너리, 모든 컴포넌트 내장 | distributor, ingester, compactor, querier 분리 |
| 적합 환경 | PoC, 소규모 (~수십 traces/s) | 프로덕션, 대규모 (~수천 traces/s) |
| 메모리 | 512MB~2GB (트래픽에 따라) | 컴포넌트별 1~4GB |
| 수평 확장 | 불가 | 컴포넌트별 독립 확장 |
| 스토리지 | filesystem (로컬 디스크) | GCS/S3/Azure Blob (오브젝트 스토리지) |

### 커뮤니티 경험치 (Monolithic mode)

| 트래픽 | 메모리 | CPU | 디스크 | 출처 |
|--------|--------|-----|--------|------|
| 유휴 (~0 traces/s) | 32 MiB | 0.03% | ~2.5 MB | 이 프로젝트 실측 |
| 소규모 (~1-10 traces/s) | 100-300 MB | 0.5-2% | 50-500 MB/day | 커뮤니티 보고 |
| 중규모 (~100 traces/s) | 500MB-1GB | 5-10% | 1-5 GB/day | 커뮤니티 보고 |
| WAL replay 피크 | WAL × 2~3배 | 일시적 spike | - | 공식 권장 |

### 이 프로젝트와 비교

| 항목 | 이 프로젝트 (PoC) | 공식 권장 방향 |
|------|------------------|--------------|
| 메모리 limit | 1 GiB | PoC 수준에서 충분. 실측 3.1% 사용 |
| WAL 설정 | 기본값 (130MB/30m) | OOM 발생 시 `max_block_bytes` 50MB로 축소 |
| 리텐션 | 72h | PoC에 적합. 프로덕션은 168h+ |
| 스토리지 | filesystem | PoC OK. 프로덕션은 오브젝트 스토리지 |

> 현재 설정(1 GiB limit)은 유휴 상태 실측 기준 32배 여유가 있어 PoC 용도로 과할 수 있다. 그러나 WAL replay 피크와 부하 시 메모리 증가를 고려하면 512 MB~1 GB 범위가 안전하다. 256 MB/512 MB에서 OOM이 발생한 경험이 있으므로, WAL에 데이터가 쌓인 상태에서의 재시작을 감안하면 1 GB를 유지하는 것이 합리적이다.
