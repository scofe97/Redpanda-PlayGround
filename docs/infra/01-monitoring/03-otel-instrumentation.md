...
| `deploy/local/` | 로컬→GCP 릴레이 | X | X | O (릴레이만) | O (Spring Boot만) | Server 3 외부IP (`34.22.78.240`) |
...
`loki.source.api`(Spring Boot 로그 수신)가 `shared/`에만 있는 이유는, 로컬 개발 시 Spring Boot가 호스트에서 실행되어 Docker 소켓으로 로그를 수집할 수 없기 때문이다. GCP에서는 Spring Boot가 Server 2에서 호스트로 실행되지만, Server 2의 Alloy는 Docker 로그만 수집하고 Spring Boot 로그는 콘솔 출력으로만 남긴다(Loki4j는 gcp 프로필에서만 활성화되며, 현재 GCP에서는 `application-gcp.yml`의 otel endpoint로 트레이스만 전송).
