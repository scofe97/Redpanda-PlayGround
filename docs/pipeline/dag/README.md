...
> **Phase 3 완료 (2026-03-22)**: 크래시 복구, 설정 외부화, Job 재시도, Stale 정리, 부분 재시작, 실패 정책(3종), DAG 이벤트 발행이 구현되었다. 파라미터 주입, 실행 컨텍스트, 프리셋 관리, DAG 시각화도 추가되었다.
...
graph TB
    ...
    subgraph Event["Event 계층"]
        style Event fill:#fce4ec,stroke:#c62828,color:#333
        DEP["DagEventProducer"]
    end

    subgraph Visualization["Visualization"]
        style Visualization fill:#e8eaf6,stroke:#3949ab,color:#333
        DGR["DagGraphResponse"]
    end
    ...
    style PR fill:#e3f2fd,stroke:#1565c0,color:#333
    style SC fill:#e3f2fd,stroke:#1565c0,color:#333
    style DS fill:#e8f5e9,stroke:#2e7d32,color:#333
    style DV fill:#f3e5f5,stroke:#7b1fa2,color:#333
    style DEP fill:#fce4ec,stroke:#c62828,color:#333
    style DGR fill:#e8eaf6,stroke:#3949ab,color:#333
...
### 2.4 ParameterResolver — 파라미터 주입

ParameterResolver는 파이프라인 실행 시 사용자가 전달한 파라미터를 Job별 configJson에 주입하는 역할이다. 세 가지 정적 메서드로 구성된다. `validate(schemas, userParams)`는 스키마 기반으로 필수 파라미터 누락을 검증하고 기본값을 적용한다. 파라미터 이름은 `[A-Za-z0-9_]`만 허용하여 인젝션을 방지하는데, 이는 configJson이 Jenkins 파라미터로 그대로 전달되기 때문이다. `resolve(template, params)`는 configJson 문자열 내의 `${PARAM_NAME}` 플레이스홀더를 실제 파라미터 값으로 치환한다. `merge(systemParams, userParams)`는 시스템 파라미터(`EXECUTION_ID`, `STEP_ORDER` 등)와 사용자 파라미터를 합성하는데, 시스템 파라미터가 항상 우선하므로 사용자가 `EXECUTION_ID`를 오버라이드할 수 없다.

### 2.5 DagEventProducer — 이벤트 발행

DagEventProducer는 Avro 직렬화된 DAG 이벤트를 Kafka 토픽(`Topics.PIPELINE_EVT_DAG_JOB`)에 발행한다. `DagExecutionCoordinator`가 Job 디스패치 시점에 `publishDagJobDispatched()`, Job 완료 시점에 `publishDagJobCompleted()`를 호출한다. 이 두 이벤트(`DagJobDispatchedEvent`, `DagJobCompletedEvent`)를 Grafana Pipeline Tracker 대시보드와 프론트엔드 LiveDagGraph가 소비하여 실시간으로 Job 시작·완료 상태를 표시하는 것이다. 이벤트 발행이 DAG 실행 흐름과 분리된 별도 컴포넌트로 존재하는 이유는 시각화 관심사를 오케스트레이션 로직에서 격리하기 위해서다.

### 2.6 DagGraphResponse — DAG 시각화 응답

DagGraphResponse는 DAG 실행 상태를 Node/Edge 구조로 변환하여 반환하는 DTO다. `nodes`와 `edges` 두 리스트로 구성되며, Grafana의 Infinity 데이터소스가 각각을 별도 프레임으로 매핑하여 Node Graph 패널에 표시한다. 프론트엔드 ReactFlow 기반 LiveDagGraph도 동일한 응답을 소비한다. Node는 `id`, `title`(Job 이름), `subTitle`(Job 타입), `mainStat`(상태), `color`(상태별 색상) 필드를 가지며, Edge는 `source`→`target` 방향 의존성을 표현한다. REST API가 이 응답을 반환하므로, 폴링 또는 이벤트 구동으로 프론트엔드가 DAG 그래프를 갱신할 수 있다.

### 2.7 SagaCompensator — 보상 트랜잭션

SagaCompensator는 파이프라인 실패 시 성공 완료된 Job을 역위상 정렬 순서로 보상한다. 역위상 정렬을 사용하는 이유는 참조 정합성 때문이다. BUILD가 GIT_CLONE에 의존하는 구조에서 BUILD 아티팩트를 먼저 제거한 뒤 GIT_CLONE 워크스페이스를 정리해야, 이미 삭제된 소스를 참조하는 보상 오류가 발생하지 않는다. 보상이 성공하면 `COMPENSATED`, 보상 자체가 실패하면 `COMPENSATION_FAILED`로 기록하고 "MANUAL INTERVENTION REQUIRED" 로그를 남긴다. 자동 복구가 불가능한 상황이므로 운영자가 직접 개입해야 한다.
...
- [04-parameter-system.md](04-parameter-system.md) — 파라미터 시스템 (ParameterResolver, 스키마, 플레이스홀더 치환)
- [05-dag-visualization.md](05-dag-visualization.md) — 시각화 및 모니터링 (DagGraphResponse, LiveDagGraph, Grafana Node Graph)
- [06-api-reference.md](06-api-reference.md) — API 레퍼런스
