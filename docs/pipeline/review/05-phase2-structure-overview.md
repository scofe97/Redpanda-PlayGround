...
│   ├── domain/
│   │   ├── PipelineDefinition.java           ← 파이프라인 정의 엔티티
│   │   ├── PipelineJob.java                  ← DAG 노드 (dependsOnJobIds)
│   │   ├── PipelineJobMapping.java           ← Job 매핑 정보
│   │   └── FailurePolicy.java               ← STOP_ALL/SKIP_DOWNSTREAM/FAIL_FAST (Phase 3)
│   ├── dto/
...
│   │   ├── PipelineJobMappingRequest.java    ← Job 매핑 요청
│   │   └── PipelineJobResponse.java          ← record
│   ├── engine/
│   │   ├── DagValidator.java                 ← Kahn's algo, 순환 탐지, 연결성 검증
│   │   ├── DagExecutionState.java            ← findReadyJobIds(), 역방향 위상 순서
│   │   └── DagExecutionCoordinator.java      ← ReentrantLock, 스레드 풀 디스패치
│   ├── event/
│   │   └── DagEventProducer.java             ← DAG Job 이벤트 발행
│   ├── mapper/
│   │   ├── PipelineDefinitionMapper.java     ← CRUD + JOIN 조회
│   │   ├── PipelineJobMapper.java            ← insertBatch, 의존성 CRUD
│   │   └── PipelineJobMappingMapper.java     ← Job 매핑 CRUD
│   └── service/
