...
> 모든 DAG 엔진 클래스는 `pipeline.dag.engine` 패키지에 위치한다. DAG 도메인은 `pipeline.dag.domain`, DAG 이벤트는 `pipeline.dag.event` 패키지를 사용한다.
...
> **ENG-1 상태**: ✅ RESOLVED — `validate()`와 `topologicalSort()` 모두 `private runKahns()`로 위임. 그래프 구축이 1회로 통합됨.
> **ENG-2 상태**: ✅ RESOLVED — `validateConnectivity()` 시그니처에서 `originalInDegree` 파라미터 삭제됨.
...
> **ENG-3 상태**: ✅ RESOLVED — record → class 전환. `markCompleted/markRunning/markFailed/markSkipped/removeRunning` 전용 변경 메서드를 제공하고, 외부 접근자(`runningJobIds()`, `failedJobIds()`)는 `Set.copyOf()`로 방어적 복사본을 반환. `skippedJobIds`, `failurePolicy` 필드도 추가됨.
...
> **ENG-4 상태**: ⚠️ OPEN — `dispatchReadyJobs()`에서 `lock.isHeldByCurrentThread()` 패턴 유지 중. `startExecution()`과 `onJobCompleted()` 모두 lock을 들고 호출하도록 통일되었으나, `dispatchReadyJobs()` 자체의 조건 분기는 제거되지 않음.
> **ENG-5 상태**: ✅ RESOLVED — `@Scheduled(fixedDelay = 300_000)` `cleanupStaleExecutions()` 추가. 5분마다 완료된 실행의 `executionStates`/`executionLocks` 정리 및 stale RUNNING 실행을 FAILED 처리. `@PostConstruct` `recoverRunningExecutions()`로 재시작 시 RUNNING 실행 복원도 구현됨.
> **ENG-6 상태**: ✅ RESOLVED — `startExecution()`에서 lock을 먼저 `executionLocks.put()` 후 즉시 `lock.lock()`하고, lock 안에서 `DagExecutionState.initialize()` + `executionStates.put()` + `dispatchReadyJobs()` 수행. race window 제거됨.
...
> **ENG-7 상태**: ⚠️ OPEN — `IMPORT → JenkinsCloneAndBuildStep` 매핑이 유지되고 있으며 주석에 `// placeholder: IMPORT 전용 executor 미구현`이 명시됨. 전용 executor 구현이 필요한 미완 항목.

## DagEventProducer — DAG 전용 이벤트 발행

`pipeline.dag.event.DagEventProducer`는 DAG Job의 dispatch/completion 이벤트를 발행한다. 기존 `PipelineEventProducer`와 분리하여 DAG 전용 이벤트(`DagJobDispatchedEvent`, `DagJobCompletedEvent`)를 관리한다. Phase 3의 Feature #7에서 추가되었다.
...
| ID | 심각도 | 설명 | 상태 | 해결 방법 |
|----|--------|------|------|----------|
| ENG-1 | MAJOR | `topologicalSort()` 중복 연산 | ✅ RESOLVED | `validate()`와 `topologicalSort()` 모두 `private runKahns()`로 위임하여 그래프 구축 1회로 통합 |
| ENG-2 | MINOR | `validateConnectivity()` 미사용 파라미터 | ✅ RESOLVED | `originalInDegree` 파라미터 시그니처에서 삭제 |
| ENG-3 | CRITICAL | `DagExecutionState` record 불변성 위반 | ✅ RESOLVED | record → class 전환, 전용 변경 메서드 제공, 외부 접근자는 `Set.copyOf()` 방어적 복사 반환 |
| ENG-4 | MAJOR | `dispatchReadyJobs()` lock reentrance 복잡성 | ⚠️ OPEN | `isHeldByCurrentThread()` 조건 분기 패턴 유지 중. 호출 경로 추가 시 데드락 위험 잠재 |
| ENG-5 | MAJOR | `executionStates`/`executionLocks` 메모리 누수 | ✅ RESOLVED | `@Scheduled` `cleanupStaleExecutions()` 5분 주기 정리 + `@PostConstruct` 크래시 복구 구현 |
| ENG-6 | MAJOR | `startExecution()` lock 없이 상태 초기화 | ✅ RESOLVED | lock 등록 직후 `lock.lock()` 후 state 초기화 + dispatch를 lock 범위 내에서 수행 |
| ENG-7 | MINOR | `IMPORT` job type placeholder 매핑 | ⚠️ OPEN | `JenkinsCloneAndBuildStep` placeholder 유지 중. IMPORT 전용 executor 미구현 |
