...
public class ExecutionJobPersistenceAdapter implements ExecutionJobPort {
    ...
    @Override
    public List<ExecutionJob> findByStatusAndMdfcnDtBefore(ExecutionJobStatus status, LocalDateTime cutoff) {
        return jpaRepository.findByStatusAndMdfcnDtBefore(status.name(), cutoff).stream()
                .map(mapper::toDomain)
                .toList();
    }
