...
#
# 검증 항목:
#   1) 최소 1개 Job이 활성 상태 (SUBMITTED/RUNNING/QUEUED/SUCCESS)
...
tc07_multi_trigger() {
    ...
    # 각 Job의 상태를 조회하여 분류
    local count_submitted=0 count_running=0 count_queued=0 count_pending=0 count_success=0
    ...
    for jid in "$job1" "$job2" "$job3"; do
        ...
        case "$s" in
            SUBMITTED) count_submitted=$((count_submitted + 1)) ;;
    ...
    local active=$((count_submitted + count_running + count_queued + count_success))
    if [ $active -ge 1 ]; then
        pass "At least 1 job is SUBMITTED/RUNNING/QUEUED/SUCCESS (active=${active})"
    else
        fail "No active job found — all may be stuck in PENDING"
    fi

    # 검증 2: 동시성 제한 관찰 (일부가 PENDING이거나 active가 제한 이하)
    if [ $count_pending -ge 1 ] || [ $((count_submitted + count_running + count_queued)) -le 1 ]; then
        pass "Concurrency control observed: pending=${count_pending}, submitted=${count_submitted}, running=${count_running}, queued=${count_queued}"
    else
